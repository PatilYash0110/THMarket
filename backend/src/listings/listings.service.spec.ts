...
describe('ListingsService favorites', () => {
  ...
  it('rejects favoriting an own listing without writing a favorite', async () => {
    prisma.listing.findUnique.mockResolvedValue({ id: 'listing', sellerId: 'owner' });

    await expect(service.addFavorite('owner', 'listing')).rejects.toBeInstanceOf(ForbiddenException);
    expect(prisma.favorite.upsert).not.toHaveBeenCalled();
  });

  it('still allows favoriting another user\'s listing', async () => {
    prisma.listing.findUnique.mockResolvedValue({ id: 'listing', sellerId: 'seller' });

    await expect(service.addFavorite('buyer', 'listing')).resolves.toBeUndefined();
  ...
  it('rejects missing listings without writing a favorite', async () => {
    ...
    await expect(service.addFavorite('buyer', 'missing')).rejects.toBeInstanceOf(NotFoundException);
    expect(prisma.favorite.upsert).not.toHaveBeenCalled();
  });

  it('allows removing an own listing favorited before this restriction', async () => {
    await expect(service.removeFavorite('owner', 'listing')).resolves.toBeUndefined();
