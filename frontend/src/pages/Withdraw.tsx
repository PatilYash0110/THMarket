...
import { withdrawBalance } from '../api/wallet'
...
export function Withdraw() {
  ...
  const [amount, setAmount] = useState(currentUser ? String(currentUser.balanceCents / 100) : '')
  ...
  if (currentUser.balanceCents === 0) {
    return (
      <div className="mx-auto flex max-w-sm flex-col items-center gap-4 py-16 text-center">
        <p className="text-sm text-foreground-muted">Kein Guthaben zum Auszahlen vorhanden.</p>
        <Link to="/profile" className="text-sm font-medium text-accent underline">
          Zurück zum Profil
        </Link>
      </div>
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    if (currentUser && amountCents > currentUser.balanceCents) {
      setError('Der Betrag übersteigt dein verfügbares Guthaben.')
      return
    }
    ...
    try {
      ...
      setError(err instanceof ApiError ? err.message : 'Auszahlung fehlgeschlagen. Bitte versuche es erneut.')
    } finally {
      setSubmitting(false)
    }
  }
    ...
    <div className="mx-auto flex max-w-md flex-col gap-8 py-8">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-5">
        ...
        <Button type="submit" size="lg" disabled={submitting}>
          {submitting ? 'Wird verarbeitet…' : 'Guthaben auszahlen'}
        </Button>
      </form>
    </div>
  )
